...
def train(args, model, device, train_loader, optimizer, epoch, epochs, LBWeight):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        attack = torchattacks.GN(model, sigma = 32./255.)
        adv_images = attack(data, target)
        confidences = F.softmax(model(adv_images), dim=1)
        output = F.log_softmax(model(adv_images), dim=1)
...
def test(model, device, test_loader, f):
...
def main():
    ...
    parser.add_argument('--model_name', type=str, default=WideResNet_1(), metavar='N',
    ...
    parser.add_argument('--LBWeight', type=float, default=0.08,
                        help='Logit Balancing Weight (default: 0.08)')
